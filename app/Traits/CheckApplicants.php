...
{
    public function checkApplicants($chat_id, $profileIds){
        ...
        if($model_name == config("constant.CHAT_MODEL_SUPPORT.COLLABORATE")){
            $applicants = Applicant::where('collaborate_id', $model_id)->pluck('profile_id')->toArray();
            ...
            $applicants = surveyApplicants::where('survey_id', $model_id)->pluck('profile_id')->toArray();
