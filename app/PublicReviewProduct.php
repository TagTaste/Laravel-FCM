...
class PublicReviewProduct extends Model
{
    ...
    public function getOverallRatingAttribute()
    {
        ...
        if($header != null)
        {
            ...
            $question = \DB::table('public_review_questions')->where('header_id',$header->header_id)->where('questions->select_type',5)->first();
            \Log::info($header->header_id);
            \Log::info(\DB::table('public_review_questions')->where('header_id',$header->header_id)->where('questions->select_type',5)->toSql());
