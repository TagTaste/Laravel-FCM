...
class Collaborate extends BaseCollaborate
{
    ...
    protected $visible = ['id', 'title', 'expires_on', 'profile_id', 'company_id', 'created_at', 'updated_at', 'deleted_at', 'video','videos_meta', 'location', 'privacy_id', 'description', 'duration', 'file1', 'state', 'step', 'type', 'type_id', 'is_taster_residence', 'collaborate_type', 'brand_name', 'brand_logo', 'methodology_id', 'no_of_batches', 'global_question_id', 'images_meta', 'applicationCount', 'type', 'product_review_meta', 'tasting_methodology', 'profile', 'company', 'categories', 'addresses', 'collaborate_specializations', 'collaborate_allergens', 'description_updated', 'document_required', 'financials', 'financial_min', 'financial_max', 'track_consistency', 'is_contest', 'max_submissions', 'mandatory_fields', 'show_interest_state', 'closing_reason', 'admin_note', 'product_review_type', 'download_report', 'view_graph'];

    protected $appends = ['description_updated', 'applicationCount', 'product_review_meta', 'type', 'tasting_methodology', 'addresses', 'mandatory_fields', 'closing_reason', 'product_review_type', 'download_report', 'view_graph'];
    ...
    public function getDescriptionUpdatedAttribute()
    {
        ...
        if (
            !is_null($this->eligibility_criteria) &&
        ...
        if (
            !is_null($this->project_commences) &&
    ...
    public function getDownloadReportAttribute()
    {
        $reportExists = \DB::table('collaborate_reports')->where('collaborate_id', $this->id)->get();
        if (!empty($reportExists)) {
            return true;
        }
    }

    public function getViewGraphAttribute()
    {
        $reportExists = \DB::table('collaborate_tasting_questions')->where('collaborate_id', $this->id)->get()->pluck('questions');
        foreach ($reportExists as $value) {
            $question = json_decode($value);
            if (isset($question->create_graph) && $question->create_graph) {
                return true;
            }
        }
    }
