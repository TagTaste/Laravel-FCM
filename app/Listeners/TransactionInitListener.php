...
class TransactionInitListener
{
    ...
    public function handle(TransactionInit $event)
    {
        ...
        $data = PaymentLinks::create(["transaction_id" => $buildTxnId, "profile_id" => request()->user()->profile->id, "model_type" => $event->data->model_type, "model_id" => $event->data->model_id, "sub_model_id" => $event->data->sub_model_id ?? NULL, "amount" => $event->data->amount, "payout_amount"=>$event->data->payout_amount,"tds_amount"=>$event->data->tds_amount,"phone" => request()->user()->profile->phone, "status_id" => config("constant.PAYMENT_INITIATED_STATUS_ID"), "payment_id" => $event->data->payment_id, "payment_channel" => $channel,"is_expert"=>request()->user()->profile->is_expert, "account_reconciliation_date"=>Carbon::now()]);

        if ($data) {
            if (!empty(request()->user()->profile->phone)) {
                $d = ["transaction_id" => $buildTxnId, "amount" => $event->data->payout_amount, "phone" => request()->user()->profile->phone, "email" => request()->user()->email, "model_type" => $event->data->model_type, "title" => $event->data->model_id, "name" => request()->user()->name ?? "", "model" => $data, "model_id" => $event->data->model_id];
