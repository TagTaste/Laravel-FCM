...
class ProfileController extends Controller
{
    ...
    public function tagging(Request $request)
    {
        ...
        $this->model = \App\Recipe\Profile::select('profiles.*')->join('users','profiles.user_id','=','users.id')->where('users.name','like',"$query%")->orWhere('users.name','like',"% $query%")->whereIn('profiles.id',$profileIds)->get();
        return $this->sendResponse();
