...
class LandingPageController extends Controller
{
    ...
    public function getSuggestion($profileId)
    {
        ...
        $quizSugges = $this->getModelSuggestionIds($client, $profileId, config("constant.LANDING_MODEL.QUIZ"));
        ...
        while ($suggCount <= 5) {
            ...
            array_push($tempMixSuggs, array_shift($quizSugges));

            if ((count($productReviewSuggs) + count($productSuggs) + count($collaborateSugges)
                + count($surveySugges) + count($pollSugges) + count($quizSugges)) == 0) {
                break;
            }
    ...
    protected function getModelSuggestionIds($client, $profileId, $modelName)
    {
        switch ($modelName) {
            ...
            case config("constant.LANDING_MODEL.SURVEYS"):
                ...
                return $data;
                break;
                case config("constant.LANDING_MODEL.QUIZ"):
                    $query = "MATCH (user:User{profile_id:$profileId})-[:FOLLOWS{following:1}]-> (users:User)-[:QUIZ_PARTICIPATION]->(quiz:Quiz)
                    WHERE NOT ((user) -[:QUIZ_PARTICIPATION]->(quiz)) AND quiz.profile_id <> $profileId
                    WITH quiz, rand() AS number
                    ORDER BY number
                    return quiz.quiz_id, quiz.payload_id LIMIT 3;";
    
                    $result = $client->run($query);
                    $data = [];
                    foreach ($result->records() as $record) {
                        array_push($data, [
                            'id' => $record->get('quiz.quiz_id'),
                            'payload_id' => $record->get('quiz.payload_id'),
                            'model_name' => $modelName
                        ]);
                    }
                    return $data;
    ...
    protected function getModelSuggestion($client, $profileId, $suggestionObj)
    {
        ...
        if ($suggestionObj['model_name'] == config("constant.LANDING_MODEL.PRODUCT")) {
            ...
            if (count($modelData) > 0) {
                ...
                else if ($modelName == config("constant.LANDING_MODEL.QUIZ")) {
                    $query = "MATCH (users:User) -[:QUIZ_PARTICIPATION]-> (quiz:Quiz{quiz_id:'$modelId'}),
                        (user:User{profile_id:$profileId})
                        WHERE ((user) -[:FOLLOWS{following:1}]-> (users))
                        WITH users, rand() as number
                        ORDER BY number   
                ...
                $subTitle = '';
                if ($modelName == config("constant.LANDING_MODEL.POLLING") || $modelName == config("constant.LANDING_MODEL.SURVEYS") || $modelName == config("constant.LANDING_MODEL.QUIZ") ) {
