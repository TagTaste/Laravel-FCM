...
class SearchController extends Controller
{
    ...
    private function filterSuggestions(&$term, $type = null, $skip, $take)
    {
        ...
        if (null == $type || "collaborate" === $type) {
            $collaborates = \DB::table('collaborates')->where('title', 'like', '%' . $term . '%')->whereNull('deleted_at')->orderBy('id', 'desc')->skip($skip)
                ->take($take)->get();
    ...
    public function filterSearch(Request $request, $type = null)
    {
        ...
        if ($suggestions && $suggestions->count()) {
            ...
            //                $this->model[$type] = [];
            //            }
            if ($type == 'collaborate' || $type == 'product' || $type == 'surveys' || $type == "quiz")
