...
class AccountManagementOptionController extends Controller
{
    ...
    public function get_user_activity(Request $request){
        ...
        $user_companies = $this->get_user_companies($request->user()->profile->id);
    ...
    function get_user_companies($profile_id){
        $data = [];

        $company_list = Company::where('user_id',$profile_id)->whereNull('deleted_at')->get();
        
        foreach($company_list as $company){
            $company_obj = ['model_id'=>$company->id, 'model_name'=>'company','name'=>$company->name, 'image_meta'=>json_decode($company->logo_meta, true)];
            $data[] = $company_obj;
        }
