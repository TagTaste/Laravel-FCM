...
trait FilterTraits
{
    ...
    public function getProfileIdOfFilter($quizDetails, Request $request)
    {
        ...
        if (!empty($filters)) {
            $Ids = QuizApplicants::where('quiz_id', $quizDetails->id)
        ...
        if (isset($filters['profile'])) {
            $Ids =   $Ids->leftJoin('profile_specializations', 'quiz_applicants.profile_id', '=', 'profile_specializations.profile_id')
        ...
        if (isset($filters['application_status'])) {
            ...
            $Ids = $Ids->where(function ($query) use ($filters) {
                foreach ($filters['application_status'] as $status) {
                    ...
                    $query->orWhere('quiz_applicants.application_status', config("constant.QUIZ_APPLICANT_STATUS." . ucwords($status)));
                }
            });
        }

        if (isset($filters['sensory_trained']) || isset($filters['super_taster']) || isset($filters['user_type'])) {
            $Ids =   $Ids->leftJoin('profiles', 'quiz_applicants.profile_id', '=', 'profiles.id');
        }
    ...
    public function getFilterParameters($quiz_id, Request $request)
    {
    ...
    public function sortApplicants($sortBy, $applications, $quizId)
    {
        $key = array_keys($sortBy)[0];
        $value = $sortBy[$key];
        if ($key == 'name') {
            $userNames = $this->getUserNames($quizId);
            $companyNames = $this->getCompanyNames($quizId);
            ...
            return $applications->orderByRaw("field(quiz_applicants.id,{$placeholders})", $order)
                ->select('quiz_applicants.*');
        }
        return $applications->orderBy('quiz_applicants.created_at', $value)->select('quiz_applicants.*');
    }

    private function getCompanyNames($id)
    {
        return QuizApplicants::where('quiz_id', $id)
            ->leftJoin('companies', function ($q) {
                $q->on('quiz_applicants.company_id', '=', 'companies.id');
            })->where('quiz_applicants.company_id', '!=', null)
            ->select('companies.name AS name', 'quiz_applicants.id')
            ->get();
    }
    ...
    private function getUserNames($id)
    {
        return QuizApplicants::where('quiz_id', $id)
            ->leftJoin('profiles AS p', function ($q) {
                $q->on('quiz_applicants.profile_id', '=', 'p.id')
                    ->where('quiz_applicants.company_id', '=', null);
            })->leftJoin('users', 'p.user_id', '=', 'users.id')->where('users.name', '!=', null)
            ->select('users.name as name', 'quiz_applicants.id')
            ->get();
    }

    public function getSearchedProfile($q, $id)
    {
        $searchByProfile = \DB::table('quiz_applicants')
            ->where('quiz_id', $id)
            ->whereNUll('company_id')
            ->join('profiles', 'quiz_applicants.profile_id', '=', 'profiles.id')
            ->join('users', 'profiles.user_id', '=', 'users.id')
            ->where('users.name', 'LIKE', '%' . $q . '%')
            ->pluck('quiz_applicants.id');

        $searchByCompany = \DB::table('quiz_applicants')
            ->where('quiz_id', $id)
            ->leftJoin('companies', 'quiz_applicants.company_id', '=', 'companies.id')
