    ...
    public function rollbackTaster(Request $request, $id)
    {
        ...
        foreach ($profileIds as $profileId) {
            ...
            if ($currentStatus == 1 || $currentStatus == 0) {
                ...
                $t = surveyApplicants::where("profile_id", $profileId)->where('survey_id', $id)->where('application_status', 1)->update(["application_status" => 0]);
                $err = false;
                if ($t) {
                    $this->model = true;
                    $survey->is_invited = $t->is_invited;
                    $survey->is_survey =1;
                } else {
                    $err = true;
                }
                ...
                $company = Company::where('id', $survey->company_id)->first();
                if (empty($company)) {
                    $who = Profile::where("id", "=", $survey->profile_id)->first();
                }
                $survey->profile_id = $profileId;
                
                event(new \App\Events\Actions\RollbackTaster($survey, $who, null, null, null, $company));
            } else {
                $err = true;
            }
        }
