...
class Collaborate extends Model implements Feedable
{
    ...
    public function getPreviewContent()
    {
        ...
        $data['owner'] = isset($profile->id) ? $profile->id : null;;
        $data['title'] = substr($this->title, 0, 65);
        $data['description'] = isset($profile->name) ? $profile->name : null;
        $data['title'] = isset($profile->name) ? $profile->name : null . ' is looking for ' . substr($this->title, 0, 65);
        $data['description'] = substr($this->description, 0, 155);
        $data['ogTitle'] = substr($this->title, 0, 65);
        $data['ogDescription'] = isset($profile->name) ? $profile->name : null;
