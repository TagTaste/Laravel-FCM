...
class ExpiryPoll extends Action
{
    ...
    public function __construct($event)
    {
        ...
        if (isset($event->model->isAdmin)) {
            ...
            $this->sub = "Your poll is closed: " . (strlen($event->model->title) > 80 ? substr($event->model->title, 0, 80) . "..." : $event->model->title);
        } else {
            
            $this->sub = htmlspecialchars_decode($event->who["name"]) . " poll is closed: " . (strlen($event->model->title) > 80 ? substr($event->model->title, 0, 80) . "..." : $event->model->title);
        }
