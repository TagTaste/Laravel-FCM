...
@section('header')
    <div class="page-header">
        <h1><i class="glyphicon glyphicon-edit"></i> Followers / Edit #{{$follower->id}}</h1>
    </div>
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{ route('followers.update', $follower->id) }}" method="POST">
                <input type="hidden" name="_method" value="PUT">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">

                <div class="form-group @if($errors->has('chef_id')) has-error @endif">
                       <label for="chef_id-field">Chef_id</label>
                    <input type="text" id="chef_id-field" name="chef_id" class="form-control" value="{{ is_null(old("chef_id")) ? $follower->chef_id : old("chef_id") }}"/>
                    ...
                    <input type="text" id="follower_id-field" name="follower_id" class="form-control" value="{{ is_null(old("follower_id")) ? $follower->follower_id : old("follower_id") }}"/>
