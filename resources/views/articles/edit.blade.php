    ...
    <form action="{{ route('articles.update', $article->id) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="col-md-9">
        ...
        <div class="form-group @if($errors->has('title')) has-error @endif">
         ...
         <input type="text" id="title-field" name="article[title]" class="form-control" value="{{ is_null(old("title")) ? $article->title : old("title")  }}"/>
         @if($errors->has("title"))
          <span class="help-block">{{ $errors->first("title") }}</span>
          @endif
       </div>
       @endif
       @include($type . "_articles.forms.edit")
     </div>
     <div class="col-md-3">
      <div class="form-group @if($errors->has('privacy_id')) has-error @endif">
       ...
       <select name="article[privacy_id]" id="" class='form-control'>
          @foreach($privacy as $name => $id)
            @if($id == $article->privacy_id)
              <option value="{{ $id }}" selected @if($id == 1) selecterd @endif> {{ $name }} </option>
            @else 
     ...
     <div class="form-group @if($errors->has('status')) has-error @endif">
       ...
       <input type="text" id="status-field" name="article[status]" class="form-control" value="{{ is_null(old("status")) ? $article->status : old("status")   }}"/>
     ...
     <div class="form-group @if($errors->has('comments_enabled')) has-error @endif">
       ...
       <div class="btn-group" data-toggle="buttons"><label class="btn btn-primary @if($article->comments_enabled == 1) active @endif"><input type="radio" value="1" name="article[comments_enabled]" id="comments_enabled-field" autocomplete="off"> True</label><label class="btn btn-primary @if($article->comments_enabled == 0) active @endif"><input type="radio" name="article[comments_enabled]" value="0" id="comments_enabled-field" autocomplete="off"> False</label></div>
